...
//==============================
    ...
    $IPKMinimal = str_replace( ',', '.', $IPKMinimal );
		...
		if ($mau_ke == "cari") {
			$a['data']		= $this->db->query("SELECT * FROM bsw_jenis WHERE NA='N' and Nama LIKE '%$cari%' OR Kode LIKE '%$cari%' ORDER BY Kode asc")->result();
...
//			$a['data']		= $this->db->query("SELECT * FROM t_import_excel WHERE isi_ringkas LIKE '%$cari%' OR indek_berkas LIKE '%$cari%' OR dari LIKE '%$cari%' OR no_surat LIKE '%$cari%' ORDER BY id DESC")->result();
 ...
 //=======================================================
  ...
  public function beasiswa_lain() {
	...
	public function beasiswa_pemkab() {
		if ($this->session->userdata('admin_valid') == FALSE && $this->session->userdata('admin_id') == "") {
			redirect("admin/login");
		}
		...
		$a['pagi']	= _page($total_row, $per_page, 4, base_url()."admin/beasiswa_pemkab/p");
    ...
    $tgl_surat     = date('Y-m-d' , strtotime($tgl_surat));
		...
		if ($mau_ke == "del") {
			...
			redirect('admin/beasiswa_pemkab');
		} else if ($mau_ke == "act_edt") {
			if ($this->upload->do_upload('file_surat')) {
				$up_data	 	= $this->upload->data();
