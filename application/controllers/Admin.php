  ...
  $Nama				  = addslashes($this->input->post('Nama'));
    ...
    if($Status=='1')
    {
      $this->db->query("UPDATE t_periode SET  Status='0' ");
     	$this->db->query("UPDATE t_periode SET  Status='$Status' WHERE PeriodeID = '$idp'");
   ...
   //Check Apakah ada periode yg aktif
    ...
    if($AdaPeriodeAktif=='1')
        ...
        {
          $this->db->query("UPDATE t_periode SET  Status='1' WHERE PeriodeID = '$idp'");
  ...
  //  $TanggalLahir     = date('Y-m-d' , strtotime($TanggalLahir));
    ...
    else
    {
     ...
     SimpanSesi('BeasiswaID','beasiswa_disetujui',$BeasiswaID);
     	...
     	$a['data']		= $this->db->query("SELECT t1.InputBy,t1.PemohonID,t1.MhswID,t1.Nama,t2.Nama as NamaProdi,t1.BeasiswaID,t1.Periode,t1.File,t1.Status FROM bsw_pemohon t1 inner join t_prodi t2 on t1.ProdiID=t2.ProdiID  WHERE $where  t1.Periode='$PeriodeAktif' and t1.NA='N' and t1.IsDeleted='N' ORDER BY t1.Periode,t1.ProdiID  DESC LIMIT $awal, $akhir ")->result();
     	$a['page']		= "l_beasiswa_disetujui";
} else if ($mau_ke == "add") {
			$q_nomer_terakhir = $this->db->query("SELECT (MAX(no_agenda)) AS last FROM t_surat_masuk WHERE YEAR(tgl_diterima) = '".$this->session->userdata('admin_ta')."'")->row_array();
			$last	= str_pad(intval($q_nomer_terakhir['last']+1), 4, '0', STR_PAD_LEFT);
...
//$Periode		= addslashes($this->input->post('Periode'));
  ...
  $uraian			= addslashes($this->input->post('uraian'));
   ...
   //Tambah data   
        ...
        VALUES ('$JenisKelamin','1', '$TanggalLahir','$NamaMhsw', '$MhswID', '$IPK', '$SKSLulus', '$Semester', '$Periode', '$Alamat', '$NoHP', '$Keterangan', '$PekerjaanOrtu', '$TanggunganOrtu', '$PenghasilanOrtu', '$ProdiID', '$KodePT', '".$up_data['file_name']."','$JenjangStudi','$TempatLahir','10')     ");
        ...
        {
         $this->db->query(" INSERT INTO `bsw_pemohon` (`JenisKelamin`,`BeasiswaID`,`TanggalLahir`, `Nama`, `MhswID`, `IPK`, `SKSLulus`, `Semester`, `Periode`, `Alamat`, `NoHP`, `Keterangan`, `PekerjaanOrtu`, `TanggunganOrtu`, `PenghasilanOrtu`, `ProdiID`, `KodePT`, `File`, `JenjangStudi`, `TempatLahir`,`Status`) 
        VALUES ('$JenisKelamin','1', '$TanggalLahir','$NamaMhsw', '$MhswID', '$IPK', '$SKSLulus', '$Semester', '$Periode', '$Alamat', '$NoHP', '$Keterangan', '$PekerjaanOrtu', '$TanggunganOrtu', '$PenghasilanOrtu', '$ProdiID', '$KodePT', '','$JenjangStudi','$TempatLahir','10')     ");
	         $this->session->set_flashdata("k", "<div class=\"alert alert-warning\" id=\"alert\">Beasiswa untuk Mahasiswa tsb dipindah ke Penampung, karena telah mendaftar di Beasiswa yg sama pada periode yg sama</div>");			
        ...
        {
         $this->db->query(" INSERT INTO `bsw_pemohon` (`JenisKelamin`,`BeasiswaID`,`TanggalLahir`, `Nama`, `MhswID`, `IPK`, `SKSLulus`, `Semester`, `Periode`, `Alamat`, `NoHP`, `Keterangan`, `PekerjaanOrtu`, `TanggunganOrtu`, `PenghasilanOrtu`, `ProdiID`, `KodePT`, `File`, `JenjangStudi`, `TempatLahir`,`Status`) 
        VALUES ('$JenisKelamin','$BeasiswaID', '$TanggalLahir','$NamaMhsw', '$MhswID', '$IPK', '$SKSLulus', '$Semester', '$Periode', '$Alamat', '$NoHP', '$Keterangan', '$PekerjaanOrtu', '$TanggunganOrtu', '$PenghasilanOrtu', '$ProdiID', '$KodePT', '','$JenjangStudi','$TempatLahir','10')     ");
	        $this->session->set_flashdata("k", "<div class=\"alert alert-success\" id=\"alert\">Data telah ditambahkan berikut upload file</div>");			
 ...
 //=======================================================
  ...
  //=======================================
    ...
    $Periode			= addslashes($this->input->post('Periode'));
		...
		if ($mau_ke == "del") {
			$this->db->query("DELETE FROM t_surat_keluarc WHERE id = '$idu'");
			$this->session->set_flashdata("k", "<div class=\"alert alert-success\" id=\"alert\">Data has been deleted </div>");
			...
			if ($this->upload->do_upload('file_surat')) {
				...
				$this->db->query("INSERT INTO t_kumpulan_sk VALUES (NULL, '$BeasiswaID', '$PeriodeID', '$Nama','$NoSK',  '".$up_data['file_name']."', '$Tgl_SK','$Status')");
			} else {
				$this->db->query("INSERT INTO t_kumpulan_sk VALUES (NULL, '$BeasiswaID', '$PeriodeID','$Nama','$NoSK',  '', '$Tgl_SK','$Status')");
			}		
			...
			if ($this->upload->do_upload('file_surat')) {
				...
				$this->db->query("UPDATE t_kumpulan_sk SET NoSK = '$NoSK', BeasiswaID = '$BeasiswaID', File = '".$up_data['file_name']."',Tgl_SK = '$Tgl_SK'  WHERE SKID = '$idp'");
			} else {
				$this->db->query("UPDATE t_kumpulan_sk SET NoSK = '$NoSK', BeasiswaID = '$BeasiswaID', Tgl_SK = '$Tgl_SK' WHERE SKID = '$idp'");
			}	
